...
class DiscriminatorZ(nn.Module):
    def __init__(self):
        ...
        dims = (consts.NUM_Z_CHANNELS, consts.NUM_ENCODER_CHANNELS, consts.NUM_ENCODER_CHANNELS // 2,# 不知道NUM_ENCODER_CHANNELS是多少，应该是64
        ...
        for epoch in range(1, epochs + 1):
            ...
            try:
                ...
                for i, (images, labels) in enumerate(train_loader, 1):
                    ...
                    print("\r{i}/{n}".format(i=i,n=len(train_loader)),end="")
                    ...
                    # losses['cons'].append(consis_los.item())
                    ...
                    labels_age = labels.clone()
                    labels_age[:, 0] = 1
                    labels_age[:, 1:10] = -1
                    labels_age[:, 10:] = labels[:, 10:]

                    # label_age_gender = torch.cat((labels_age, labels[:, 10:]), 1)
                    # constructed_z = self.E(images)
                    constructed_z_l = torch.cat((z, labels_age), 1)
                    constructed_img = self.G(constructed_z_l)
                    raw_z = self.E(constructed_img)
                    raw_z_l = torch.cat((raw_z, labels), 1)
                    raw_img = self.G(raw_z_l)
                    consis_los = torch.mean(torch.abs(raw_img - images))
                    for i in range(1,10):
                        # labels_age = torch.FloatTensor(batch_size,10)
                        labels_age = labels.clone()
                        labels_age[:, i] = 1
                        labels_age[:, :i] = -1
                        labels_age[:, (i + 1):10] = -1
                        ...
                        raw_z_l = torch.cat((raw_z, labels), 1)
                        raw_img = self.G(raw_z_l)
                        consis_los = consis_los + torch.mean(torch.abs(raw_img-images))
                        # self.cycle_loss = tf.reduce_mean(tf.abs(self.input_image - self.Generator[0]))
                        # for i in range(1, num_categories - 1):
                        #     self.cycle_loss = self.cycle_loss + tf.reduce_mean(
                        #         tf.abs(self.input_image - self.Generator[i]))
                        # consis_loss = consis_los / 10.0
                    ...
                    loss = eg_loss + reg_loss + ez_loss + dg_loss + consis_loss
                ...
                print_timestamp("[Epoch {epoch:d}] Loss: {loss.item():f}".format(d=epoch, f=loss.item()))
